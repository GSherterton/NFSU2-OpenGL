...
void Door::drawDoor(){
  ...
  { // surfaces
    ...
    vector<array<array<array<GLfloat, 3>, 4>, 4>> window_ctrlpoints = {
      {{ // door (down - up)
        {{ // frontside
          {front_axle_shift-(tire_radius+3*fender_shift), car_hood, 0.0},
          {windshield_shift                             , car_roof, 0.0},
    ...
    for(int i = 0; i < window_ctrlpoints.size(); i++){
      glMap2f(GL_MAP2_VERTEX_3, 0, 1, 3, 4,
            0, 1, 12, 4, &window_ctrlpoints[i][0][0][0]);
